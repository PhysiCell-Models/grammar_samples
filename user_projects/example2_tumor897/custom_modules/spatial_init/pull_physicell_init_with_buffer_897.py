...
import math
import random
filename = r"/Users/morgana/Documents/GitHub/BCH_inverse/PhysiCell/user_projects/BCH_slate/custom_modules/spatial_init/coord897.csv"
fields = []
rows = []
...
for i in range(0, len(bucket_list)):
    ...
    midsection_max_index = 0
    for j in range(0, len(bucket_list[i])):
        current_y = float(bucket_list[i][j][2])
        if (current_y > y_max):
            y_max = current_y
            midsection_max_index = j
        if (current_y < y_min):
            y_min = current_y
            midsection_min_index = j
    y_min += -1 * VISIUM_spacing
    y_max += VISIUM_spacing
    x_anch_top = float(bucket_list[i][midsection_max_index][1])
    x_anch_bottom = float(bucket_list[i][midsection_min_index][1])
    while (np.sqrt(np.square(x_anch_top - x_c) + np.square(y_max - y_c)) < tissue_rad * b_strength):
        cell_frame.append([str(x_anch_top), str(y_max), '0', '3'])
        y_max += VISIUM_spacing
    while (np.sqrt(np.square(x_anch_bottom - x_c) + np.square(y_min - y_c)) < tissue_rad * b_strength):
...
# write to file
target_file_name = r"/Users/morgana/Documents/GitHub/BCH_inverse/PhysiCell/user_projects/BCH_slate/config/three_state_hypoxia_buffered_cells_897.csv"
